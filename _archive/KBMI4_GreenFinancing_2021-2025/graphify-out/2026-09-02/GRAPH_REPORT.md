# Graph Report - Skripsi  (2026-09-02)

## Corpus Check
- 67 files · ~1,356,890 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1667 nodes · 1745 edges · 97 communities (86 shown, 11 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 2 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8a0b3e14`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- script.js
- BAB V  PENUTUP ........................................................................................... 56
- BAB 4\\[0.3cm]ANALISIS DAN PEMBAHASAN
- Konseptualisasi Variabel Penelitian
- (322015175)
- Buku Pedoman Penyusunan Skripsi FEB Ukrida 2022.md
- Bab ini terdiri dari sub bab berikut ini:
- :
- Tahapan Analisis Data dan Uji Ekonometrika
- PROPOSAL_SKRIPSI_LENGKAP.md
- Source of Truth Skripsi FEB UKRIDA
- 📊 EKONOMETRIKA DATA PANEL UNTUK PEMULA TOTAL
- Blueprint Metodologi Fase 3
- BAB 1\\[0.3cm]PENDAHULUAN
- ANALISIS DAN PEMBAHASAN
- 3.5. Metode Analisis Data
- Playbook Skripsi S1 Manajemen FEB UKRIDA — Pengetahuan Lintas Topik
- 2.1. Teori-teori
- SKRIPSI_LENGKAP_BAB_1_SAMPAI_5.md
- 6 Agustus 2026
- Product Requirements Document (PRD) — Versi 2.0
- Paket Eksekusi Expert Review, Cognitive Interview, dan Field Pilot
- Search Log Literatur — Fase 3
- ABSTRAK
- 3. Scope of Remediation & Detailed Feature Requirements
- Draf Instrumen Survei dan Codebook
- 🔥 PERTANYAAN SIDANG SULIT LANJUTAN (10 Skenario Tambahan)
- 12. SIMULASI PERTANYAAN SIDANG & KUNCI JAWABAN
- Spesifikasi Analisis Fase 3
- Audit Kesiapan Operasional Sebelum Pilot — Fase 3
- Kamus Data dan Skema Relasional
- 🃏 FLASHCARD ISTILAH & KONSEP SKRIPSI
- 🎭 SIMULASI DIALOG SIDANG PENUH
- ANALISIS DAN PEMBAHASAN
- 2. Agent Roles & Detailed Work Packages
- Struktur laporan
- Kasus Uji Logika Form dan Pipeline
- BAB 5\\[0.3cm]PENUTUP
- Methodology Blueprint — Rancangan Metodologi Penelitian Skripsi
- 📊 Ringkasan 6 Fase Perbaikan yang Telah Selesai Diterapkan
- Latar Belakang Penelitian
- PANDUAN_BELAJAR_SKRIPSI_v2.md
- PENDAHULUAN
- 5.3. Saran
- Analysis Specification — Spesifikasi Ekonometrika Data Panel
- Reviewer Skripsi UKRIDA
- Daftar pemeriksaan
- Audit Isi dan Metodologi
- Search Log Skripsi — Protokol Pencarian Literatur
- Master Revision Plan: Graph of Agents (GoA) Orchestration
- 2.1. Teori-teori
- Matriks Literatur Skripsi
- Register Input dan Bukti Manusia — Fase 3
- Econometric Results Master Summary (v2.0)
- 06_RINGKASAN_PER_BAB_ONE_PAGER.md
- 8. METODOLOGI PENELITIAN & EKONOMETRIKA
- PENDAHULUAN
- METODE PENELITIAN
- Tugas: Buat Website Portofolio Sederhana (HTML + CSS + JS vanilla)
- Tugas: Percantik Website Portofolio Skripsi (iterasi 2)
- SKRIPSI_ARTHUR_LENGKAP_PRISM.md
- 1. RINGKASAN EKSEKUTIF & PETA KONSEP PENELITIAN
- Data Dictionary & Audit Dataset Skripsi
- Source of Truth Skripsi — Status Lengkap (Bab 1–5)
- 06_Review_&_Audit/README.md
- Audit Bahasa, Sitasi, dan Daftar Pustaka
- 📝 BANK SOAL LATIHAN SKRIPSI — 30 SOAL (PG + ESSAY)
- 7. LOGIKA HUBUNGAN KAUSALITAS & HIPOTESIS
- 3. KAMUS LENGKAP ISTILAH & KONSEP DASAR
- 9. HASIL ANALISIS DATA & INTERPRETASI ANGKA
- TINJAUAN PUSTAKA DAN PENGEMBANGAN HIPOTESIS
- PRISM AI Technical Review & Comprehensive Audit Synthesis
- Workflow Log Skripsi — Topik Aktif
- 11. KESIMPULAN, KETERBATASAN, & SARAN
- 2. RUMUSAN MASALAH, TUJUAN & MANFAAT PENELITIAN
- PENUTUP
- Literature Matrix — Sintesis Literatur Skripsi
- Format Laporan Hasil Review
- update_identity_all_files.py
- 🎓 PANDUAN BELAJAR & PENGUASAAN MATERI SKRIPSI (MASTER STUDY GUIDE) — v2
- run_econometrics_pipeline.py
- run_econometrics_pure.py
- KATA PENGANTAR
- Decision Log Fase 3
- patch_appendix2.py
- patch_bab5_and_tables.py
- patch_econometrics_and_prose.py
- patch_skripsi.py
- JAKARTA 2021
- JAKARTA 2021
- BAB II TINJAUAN PUSTAKA ............................................................................. 9
- BAB IV ANALISIS DATA DAN PEMBAHASAN ............................................. 37
- generate_template_word.py
- sync_markdown_from_tex.py
- 📥 FOLDER DROPZONE PENGUMPULAN LAPORAN KEUANGAN & SUSTAINABILITY REPORT
- organize_and_audit_reports.py

## God Nodes (most connected - your core abstractions)
1. `BAB V  PENUTUP ........................................................................................... 56` - 413 edges
2. `(322015175)` - 152 edges
3. `Bab ini terdiri dari sub bab berikut ini:` - 60 edges
4. `:` - 57 edges
5. `build_document()` - 26 edges
6. `set_run_font()` - 22 edges
7. `add_paragraph()` - 22 edges
8. `add_heading_bab()` - 21 edges
9. `Source of Truth Skripsi FEB UKRIDA` - 17 edges
10. `6 Agustus 2026` - 17 edges

## Surprising Connections (you probably didn't know these)
- None detected - all connections are within the same source files.

## Import Cycles
- None detected.

## Communities (97 total, 11 thin omitted)

### Community 0 - "script.js"
Cohesion: 0.23
Nodes (12): animateCountUp(), handleNavbarScroll(), handleResize(), init(), initBackToTop(), initBarAnimations(), initDynamicYear(), initMobileMenu() (+4 more)

### Community 1 - "BAB V  PENUTUP ........................................................................................... 56"
Cohesion: 0.00
Nodes (413): 01(06), 99–115., 1.1 Latar Belakang, 1.2 Perumusan Masalah, 1.3 Tujuan Penelitian, 1.4.1 Manfaat Teoritis, 1.4.2 Manfaat Praktis, 1.4 Manfaat Penelitian, 1. Convergent Validity (+405 more)

### Community 2 - "BAB 4\\[0.3cm]ANALISIS DAN PEMBAHASAN"
Cohesion: 0.05
Nodes (37): Analisis Keunggulan Bersaing Berkelanjutan Antar-Entitas Bank KBMI 4, Analisis Sensitivitas dan Elastisitas Parsial ROA terhadap Variabel Independen, Analisis Statistik Deskriptif, Analisis Tren Kuartalan dan Profil Finansial Perbankan KBMI 4 (2021–2025), BAB 4\\[0.3cm]ANALISIS DAN PEMBAHASAN, Evaluasi Ketahanan Perbankan KBMI 4 Menghadapi Volatilitas Makroekonomi, Gambaran Umum Objek Penelitian, Hasil Estimasi Regresi Data Panel (Fixed Effect Model) (+29 more)

### Community 4 - "Konseptualisasi Variabel Penelitian"
Cohesion: 0.08
Nodes (25): *Agency Theory, Analisis Komparatif Pembiayaan Hijau: Negara Berkembang vs Negara Maju, Analisis Kualitas Aset, Kolektibilitas, dan Implementasi PSAK 71 / IFRS 9, BAB 2\\[0.3cm]TINJAUAN PUSTAKA DAN PENGEMBANGAN HIPOTESIS, Dinamika Integrasi ESG ke dalam Penilaian Risiko Kredit Perbankan, Integrasi Risiko Iklim Fisik dan Transisi ke dalam Manajemen Risiko Perbankan, Kecukupan Modal dan *Capital Adequacy Ratio, Konseptualisasi Variabel Penelitian (+17 more)

### Community 5 - "(322015175)"
Cohesion: 0.01
Nodes (152): 1.1. Latar Belakang Penelitian, 1.1. Latar Belakang Penelitian, 1.1. Latar Belakang Penelitian, 1.1. Latar Belakang Penelitian, 1.2. Perumusan Masalah, 1.2. Perumusan Masalah, 1.2. Perumusan Masalah, 1.2. Perumusan Masalah (+144 more)

### Community 6 - "Buku Pedoman Penyusunan Skripsi FEB Ukrida 2022.md"
Cohesion: 0.03
Nodes (66): 1.1. Pendahuluan, 1.2. Persyaratan Penyusunan Skripsi, 1.3. Prosedur Penyusunan Skripsi, 1.4. Proposal Skripsi, 1.5. Jangka Waktu Penyusunan Skripsi, 1) Bagian awal, 2.1. Bagian Awal, 2.2. Bagian Isi (+58 more)

### Community 7 - "Bab ini terdiri dari sub bab berikut ini:"
Cohesion: 0.03
Nodes (60): 2015. Jakarta: Ikatan Akuntan Indonesia, 2.3. Bagian Akhir, 2. Letak: sudut kanan bawah kertas. Pada bagian ini, pilih insert page number, 3.1. Sampul Depan, 3.2. Pengetikan, 3.3. Penomoran Halaman, 3.4. Halaman Sampul, 3.5. Persamaan Matematika (+52 more)

### Community 8 - ":"
Cohesion: 0.04
Nodes (53): :, (312022015), (312022015), (312022015), Anggota Penguji, Anggota Penguji, BAB I PENDAHULUAN ....................................................................................... 1, BAB III METODE PENELITIAN ........................................................................ 31 (+45 more)

### Community 9 - "Tahapan Analisis Data dan Uji Ekonometrika"
Cohesion: 0.10
Nodes (21): Analisis Statistik Deskriptif, BAB 3\\[0.3cm]METODE PENELITIAN, Desain Penelitian dan Paradigma Keilmuan, Formulasi Sintaks Perintah EViews dan Alur Kerja Estimasi, Jenis Data, Jenis, Sumber, dan Teknik Pengumpulan Data, Operasionalisasi Variabel, Populasi Penelitian (+13 more)

### Community 10 - "PROPOSAL_SKRIPSI_LENGKAP.md"
Cohesion: 0.06
Nodes (35): 1.1. Latar Belakang Penelitian, 1.2. Perumusan Masalah, 1.3. Tujuan Penelitian, 1.4. Manfaat Penelitian, 1. Aspek Teoritis, 2.1.1. Teori Pemangku Kepentingan (*Stakeholder Theory*), 2.1.2. Teori Legitimasi (*Legitimacy Theory*), 2.1.3. Teori Intermediasi Finansial (*Financial Intermediation Theory*) (+27 more)

### Community 11 - "Source of Truth Skripsi FEB UKRIDA"
Cohesion: 0.08
Nodes (25): 10. Metode yang Sedang Diuji, 11. Keputusan Fase 3 Saat Ini, 12. Blocker yang Memerlukan Bukti Pengguna, 12A. Artefak Kerja yang Dapat Diambil Ulang, 13. Comparator Kakak Tingkat, 14. Kontrol Google Docs dan Format Final, 15. Integritas Akademik, 1. Urutan Otoritas (+17 more)

### Community 12 - "📊 EKONOMETRIKA DATA PANEL UNTUK PEMULA TOTAL"
Cohesion: 0.09
Nodes (22): 1. APA ITU DATA PANEL? (Analogi: Rapor Sekolah), 2. TIGA MODEL DATA PANEL (Analogi: Kelas Bimbel), 3. UJI PEMILIHAN MODEL (Analogi: Turnamen Eliminasi), 4. UJI ASUMSI KLASIK (Analogi: Tes Kesehatan Sebelum Bertanding), 5. CARA BACA OUTPUT REGRESI (Langkah Demi Langkah), 6. RANGKUMAN VISUAL KESELURUHAN, *Dari nol sampai paham — tanpa rumus yang bikin pusing, pakai analogi kehidupan nyata*, 📊 EKONOMETRIKA DATA PANEL UNTUK PEMULA TOTAL (+14 more)

### Community 13 - "Blueprint Metodologi Fase 3"
Cohesion: 0.09
Nodes (21): 10. Privasi dan Integritas, 11. Blocker Sebelum Gerbang Lulus, 1. Estimand dan Batas Inferensi, 2. Unit Analisis dan Informan, 3. Sampling yang Realistis, 4. X: Kategori Durasi Penyangga Likuid, 5. Y: Beban Kewajiban Pembayaran Kontraktual Historis, 6. DAG dan Kontrol (+13 more)

### Community 14 - "BAB 1\\[0.3cm]PENDAHULUAN"
Cohesion: 0.22
Nodes (9): 1. Manfaat Teoretis (Aspek Akademis), 2. Manfaat Praktis (Aspek Terapan), BAB 1\\[0.3cm]PENDAHULUAN, Identifikasi Masalah, Manfaat Penelitian, Pembatasan Masalah, Perumusan Masalah, Sistematika Penulisan Skripsi (+1 more)

### Community 15 - "ANALISIS DAN PEMBAHASAN"
Cohesion: 0.10
Nodes (20): 4.1. Deskripsi Sampel Penelitian, 4.2. Statistik Deskriptif, 4.3.1. Hasil Uji Pemilihan Model Regresi Data Panel, 4.3.2. Hasil Uji Asumsi Klasik, 4.3. Hasil Uji Pemilihan Model dan Uji Asumsi Klasik, 4.4. Hasil Pengujian Hipotesis, 4.5.1. Pembahasan Pengaruh *Green Financing* terhadap *Return on Assets* (ROA), 4.5.2. Pembahasan Pengaruh *Non-Performing Loan* (NPL) terhadap *Return on Assets* (ROA) (+12 more)

### Community 16 - "3.5. Metode Analisis Data"
Cohesion: 0.10
Nodes (19): 3.1.1. Jenis Data, 3.1.2. Sumber Data dan Periode Penelitian, 3.1. Jenis dan Sumber Data, 3.2.1. Populasi Penelitian, 3.2.2. Teknik Pengambilan Sampel, 3.2.3. Sampel Final Penelitian, 3.2. Populasi dan Sampel, 3.3. Model Penelitian (+11 more)

### Community 17 - "Playbook Skripsi S1 Manajemen FEB UKRIDA — Pengetahuan Lintas Topik"
Cohesion: 0.10
Nodes (19): 10. Data Sintetis dan Integritas, 11. Peran Agen, 12. Comparator dan Pedoman UKRIDA, 13. Quick Start Topik Baru, 14. Checklist Sebelum Menyebut “Layak”, 1. Urutan Otoritas, 2. Gerbang Empat Fase, 3. Prinsip Source of Truth (+11 more)

### Community 18 - "2.1. Teori-teori"
Cohesion: 0.11
Nodes (18): 2.1.1. Teori Pemangku Kepentingan (*Stakeholder Theory*), 2.1.2. Teori Legitimasi (*Legitimacy Theory*), 2.1.3. Teori Intermediasi Finansial (*Financial Intermediation Theory*), 2.1.4. Konsep Profitabilitas Bank dan *Return on Assets* (ROA), 2.1.5. Portofolio Kredit Hijau (*Green Financing*), 2.1.6. Risiko Kredit dan *Non-Performing Loan* (NPL), 2.1.7. Kecukupan Modal dan *Capital Adequacy Ratio* (CAR), 2.1. Teori-teori (+10 more)

### Community 19 - "SKRIPSI_LENGKAP_BAB_1_SAMPAI_5.md"
Cohesion: 0.11
Nodes (18): ABSTRACT, ABSTRAK, BAB 1, BAB 2, BAB 3, BAB 4, BAB 5, DAFTAR ISI (+10 more)

### Community 20 - "6 Agustus 2026"
Cohesion: 0.11
Nodes (18): 6 Agustus 2026, Audit Literatur Kedua dan Koreksi, Audit Pengganti dan Penutupan Tiga Celah Internal, Audit Substantif dan Penyederhanaan 10 Agustus 2026, Blueprint Metodologi dan Instrumen Fase 3, Fase 1, Fase 2, Fase 3 (+10 more)

### Community 21 - "Product Requirements Document (PRD) — Versi 2.0"
Cohesion: 0.11
Nodes (17): 0. Catatan Perubahan dari v1.1, 10. Contoh Interaksi — Mode Pembimbingan, Fase 1, 11. Contoh Interaksi — Mode Audit, 1. Latar Belakang, 2. Hierarki Otoritas (Berlaku di Kedua Mode), 3.1 Fitur Inti Mode Pembimbingan, 3.2 Mode Data dan Integritas, 3. Mode Pembimbingan — Empat Gerbang Fase (+9 more)

### Community 22 - "Paket Eksekusi Expert Review, Cognitive Interview, dan Field Pilot"
Cohesion: 0.12
Nodes (15): 1. Urutan Wajib, 2. Paket Expert Review, 3. Panduan Cognitive Interview, 4. Gate Sebelum Field Pilot, 5. Uji Teknis Form, 6. Flow dan Denominator Field Pilot, 7. Output Wajib Field Pilot, 8. Power dan Freeze (+7 more)

### Community 23 - "Search Log Literatur — Fase 3"
Cohesion: 0.12
Nodes (15): 1. Identitas penelusuran, 2. Protokol yang dapat direplikasi, 3. Tahapan penyaringan, 3A. Hasil pencarian aktual dan keterbatasan akses, 4. Ledger verifikasi sumber prioritas, 5. Temuan sintesis dan batas klaim research gap, 6. Risiko pengukuran yang harus diselesaikan sebelum instrumen final, 7. Daftar verifikasi lanjutan (+7 more)

### Community 24 - "ABSTRAK"
Cohesion: 0.14
Nodes (14): a positive and significant effect on customer loyalty. Furthermore, customer, ABSTRACT, ABSTRAK, customer loyalty. These findings emphasize the important role of customer, DAFTAR ISI, Equation Modeling–Partial Least Squares (SEM-PLS) method with the assistance, Glad2Glow products in West Jakarta. The data were analyzed using the Structural, method through questionnaires distributed to 130 respondents who are users of (+6 more)

### Community 25 - "3. Scope of Remediation & Detailed Feature Requirements"
Cohesion: 0.14
Nodes (13): 1.1 Background & Context, 1.2 Core Objective, 1. Project Overview & Problem Statement, 2.1 Invariant System Contracts (Unbreakable Rules), 2.2 Non-Goals, 2. Invariant Contracts & Non-Goals, 3. Scope of Remediation & Detailed Feature Requirements, 4. Definition of Done (Acceptance Criteria) (+5 more)

### Community 26 - "Draf Instrumen Survei dan Codebook"
Cohesion: 0.14
Nodes (13): A. Parameter yang Diisi Sebelum Survei, B. Pembukaan dan Persetujuan, C. Screening dan Deduplikasi, D. Definisi yang Ditampilkan, Draf Instrumen Survei dan Codebook, E. Konstruk X, F. Roster Utang dan Konstruk Y, G. Pendapatan Denominator (+5 more)

### Community 27 - "🔥 PERTANYAAN SIDANG SULIT LANJUTAN (10 Skenario Tambahan)"
Cohesion: 0.15
Nodes (12): ❓ Pertanyaan 13: "Semua hipotesis Anda diterima. Bukankah ini terlalu 'sempurna'? Apakah tidak ada yang ditolak? Bagaimana Anda menjelaskan ini?", ❓ Pertanyaan 14: "Bagaimana jika hasil Green Financing justru negatif terhadap ROA? Apa penjelasannya?", ❓ Pertanyaan 15: "Anda menggunakan Fixed Effect Model. Bisa jelaskan secara intuitif apa bedanya FEM dengan REM, tanpa rumus?", ❓ Pertanyaan 16: "Mengapa Anda tidak memasukkan variabel BOPO atau LDR yang juga sangat umum dalam penelitian perbankan?", ❓ Pertanyaan 17: "Periode 2021–2025 itu mencakup masa pandemi dan pasca-pandemi. Apakah ada kemungkinan data Anda terdistorsi oleh efek COVID-19, terutama pada NPL?", ❓ Pertanyaan 18: "Koefisien Green Financing hanya +0,042 — sangat kecil dibanding NPL (-0,385). Apakah Green Financing benar-benar penting secara ekonomis, atau hanya signifikan secara statistik saja?", ❓ Pertanyaan 19 (Follow-up dari Q5 tentang NPL): "Anda bilang NPL punya 'double impact'. Kalau begitu, apakah hubungan NPL dan ROA bersifat linear atau mungkin non-linear (threshold effect)?", ❓ Pertanyaan 20 (Follow-up dari Q7 tentang Adjusted R²): "29,8% sisanya tidak dijelaskan model Anda. Variabel apa yang menurut Anda paling kuat dari faktor-faktor 'sisa' tersebut?" (+4 more)

### Community 28 - "12. SIMULASI PERTANYAAN SIDANG & KUNCI JAWABAN"
Cohesion: 0.15
Nodes (13): 12. SIMULASI PERTANYAAN SIDANG & KUNCI JAWABAN, ❓ Pertanyaan 10 *(BARU)*: "Apa keterbatasan utama dari penelitian Anda?", ❓ Pertanyaan 11 *(BARU)*: "Apa kontribusi atau novelty penelitian Anda dibandingkan penelitian-penelitian sejenis yang sudah ada, misalnya Pramono et al. (2022) atau Wulandari & Rahardjo (2022)?", ❓ Pertanyaan 12 *(BARU)*: "Mengapa Anda menggunakan data kuartalan (triwulanan) dan bukan data tahunan atau bulanan?", ❓ Pertanyaan 1: "Mengapa Anda memilih Bank KBMI 4 sebagai objek penelitian, mengapa tidak seluruh bank di BEI?", ❓ Pertanyaan 2: "Mengapa Anda memilih proksi ROA untuk mengukur profitabilitas, bukan ROE atau NIM?", ❓ Pertanyaan 3: "Secara teori, menyalurkan Kredit Hijau membutuhkan biaya audit yang mahal dan suku bunganya seringkali bersaing/rendah. Mengapa hasil penelitian Anda justru membuktikan Green Financing berpengaruh positif terhadap ROA?", ❓ Pertanyaan 4: "Mengapa dalam pengujian data panel, Anda memilih Fixed Effect Model (FEM) dan bukan Random Effect Model (REM)?" (+5 more)

### Community 29 - "Spesifikasi Analisis Fase 3"
Cohesion: 0.15
Nodes (12): 10. Power dan Freeze, 11. Aturan Bahasa Hasil, 1. Estimand, 2. Derivasi Variabel, 3. Flow dan Eksklusi, 4. Statistik Deskriptif Wajib, 5. Model Utama Kandidat, 6. Audit Denominator Wajib (+4 more)

### Community 30 - "Audit Kesiapan Operasional Sebelum Pilot — Fase 3"
Cohesion: 0.15
Nodes (12): 1. Kesimpulan singkat, 2. Ketidaksesuaian atau keputusan yang belum ditutup, 3. Item instrumen yang ambigu atau terlalu berat, 4. Checklist eksekusi minimum, 5. Keputusan audit, A. Sebelum cognitive interview, Ambigu/berisiko salah klasifikasi, Audit Kesiapan Operasional Sebelum Pilot — Fase 3 (+4 more)

### Community 31 - "Kamus Data dan Skema Relasional"
Cohesion: 0.17
Nodes (11): 10. Pemeriksaan Logika Minimum, 1. Struktur Tabel, 2. Kunci dan Metadata, 3. Consent dan Screening, 4. Konstruk X, 5. Debt Contract, 6. Debt Month, 7. Income Month (+3 more)

### Community 32 - "🃏 FLASHCARD ISTILAH & KONSEP SKRIPSI"
Cohesion: 0.18
Nodes (10): 🃏 FLASHCARD ISTILAH & KONSEP SKRIPSI, *Format: Pertanyaan (Q) → Jawaban (A) — Baca Q dulu, coba jawab dalam hati, baru buka A*, KATEGORI 1: LEMBAGA & KLASIFIKASI, KATEGORI 2: ISTILAH OPERASIONAL & RISIKO BANK, KATEGORI 3: ISTILAH KEBERLANJUTAN (ESG & GREEN FINANCE), KATEGORI 4: VARIABEL PENELITIAN, KATEGORI 5: REGULASI, KATEGORI 6: TEORI-TEORI (+2 more)

### Community 33 - "🎭 SIMULASI DIALOG SIDANG PENUH"
Cohesion: 0.18
Nodes (10): 🎭 SIMULASI DIALOG SIDANG PENUH, SKENARIO 1: Pembukaan & Presentasi, SKENARIO 2: Penguji Menggali Pemahaman Teori, SKENARIO 3: Penguji Menyerang Metodologi, SKENARIO 4: Penguji Mempertanyakan Angka, SKENARIO 5: Penguji Menguji Pemahaman Ekonometrika, SKENARIO 6: Penguji Bertanya Tentang Implikasi Praktis, SKENARIO 7: Penguji Bertanya Tentang Keterbatasan (+2 more)

### Community 34 - "ANALISIS DAN PEMBAHASAN"
Cohesion: 0.18
Nodes (11): 4.1. Deskripsi Sampel Penelitian, 4.2. Statistik Deskriptif, 4.3.1. Uji Pemilihan Model Data Panel, 4.3.2. Uji Asumsi Klasik, 4.3. Hasil Uji Pemilihan Model dan Asumsi Klasik, 4.4. Hasil Pengujian Hipotesis, 4.5. Pembahasan, ANALISIS DAN PEMBAHASAN (+3 more)

### Community 35 - "2. Agent Roles & Detailed Work Packages"
Cohesion: 0.33
Nodes (6): 2. Agent Roles & Detailed Work Packages, Track 1: Agent Data Engineer (Data Provenance & Dataset Reconstruction), Track 2: Agent Econometrician (Panel Analysis & Diagnostic Suite), Track 3: Agent Accounting & Regulatory Specialist, Track 4: Agent Literature & Bibliographer, Track 5: Agent LaTeX Lead Typesetter

### Community 36 - "Struktur laporan"
Cohesion: 0.18
Nodes (11): 10. Pemeriksaan akhir, 1. Ringkasan eksekutif, 2. Ruang lingkup dan keterbatasan, 3. Matriks kepatuhan, 4. Daftar temuan terprioritas, 5. Audit konsistensi penelitian, 6. Audit format, 7. Audit metodologi dan analisis (+3 more)

### Community 37 - "Kasus Uji Logika Form dan Pipeline"
Cohesion: 0.18
Nodes (10): Aturan Umum, Double Count, Arrears, dan Restrukturisasi, Kasus Screening, Kasus Uji Logika Form dan Pipeline, Kasus X, Kasus Y Sederhana, Kriteria Lulus Dry Run, Mixed Purpose dan Joint Debt (+2 more)

### Community 38 - "BAB 5\\[0.3cm]PENUTUP"
Cohesion: 0.22
Nodes (9): BAB 5\\[0.3cm]PENUTUP, Kesimpulan, Keterbatasan Penelitian, Matriks Program Aksi Perbankan KBMI 4 Berkelanjutan, Peta Jalan Implementasi Strategis Perbankan Berkelanjutan (2026–2030), Saran, Saran Akademis bagi Peneliti Selanjutnya, Saran Kebijakan bagi Regulator (Otoritas Jasa Keuangan & Bank Indonesia) (+1 more)

### Community 39 - "Methodology Blueprint — Rancangan Metodologi Penelitian Skripsi"
Cohesion: 0.20
Nodes (9): 1. Desain Penelitian & Paradigma Keilmuan, 2. Populasi dan Prosedur Penentuan Sampel, 3. Spesifikasi Model Ekonometrika Data Panel, 4. Tahapan Analisis Data Panel Lengkap, 5. Kriteria Pengujian Statistik & Pengambilan Keputusan, a. Populasi Penelitian, b. Teknik Pengambilan Sampel (Purposive Sampling), c. Sampel Final (+1 more)

### Community 40 - "📊 Ringkasan 6 Fase Perbaikan yang Telah Selesai Diterapkan"
Cohesion: 0.22
Nodes (9): 1. Pembersihan Kontradiksi Internal Angka Regresi (Fase 1), 2. Standardisasi Bahasa Kausalitas $\rightarrow$ Asosiatif (Fase 2), 3. Perbaikan Uji Asumsi Klasik & Diagnostik Ekonometrika (Fase 3), 4. Koreksi Definisi Teknis Teori Keuangan & Akuntansi (Fase 4), 5. Pembersihan Sintaks Markdown dalam LaTeX (Fase 5), 6. Kebersihan Sitasi & Bibliografi (Fase 6), 📌 Di Mana Anda Bisa Melihat Hasilnya?, Laporan Hasil Revisi & Remediasi Skripsi (Berdasarkan Audit PRISM AI) (+1 more)

### Community 41 - "Latar Belakang Penelitian"
Cohesion: 0.33
Nodes (6): Dinamika Kebijakan Insentif Makroprudensial Hijau Bank Indonesia dan OJK (2021–2025), Dinamika Makroekonomi dan Transisi Pasca-Pandemi Periode 2021–2025, Latar Belakang Penelitian, Peran Sistemik Bank KBMI 4 dalam Perekonomian Nasional, Perbandingan Kerangka Regulasi Keuangan Berkelanjutan Global dan Regional, Perdebatan Empiris dan Teoretis (*Research Gap

### Community 42 - "PANDUAN_BELAJAR_SKRIPSI_v2.md"
Cohesion: 0.22
Nodes (8): 10. KONTEKS MAKROEKONOMI & PERBANKAN 2021–2025, 13. CHEAT SHEET 1 HALAMAN, 4. BEDAH VARIABEL PENELITIAN, 5. TEORI-TEORI UTAMA & KAITANNYA, 6. PENELITIAN TERDAHULU & RESEARCH GAP, Matriks Penelitian Sebelumnya, Posisi & Kontribusi Skripsi Ini, Sumber Data Tiap Variabel (sering ditanya saat sidang)

### Community 43 - "PENDAHULUAN"
Cohesion: 0.22
Nodes (8): 1.1. Latar Belakang Penelitian, 1.2. Perumusan Masalah, 1.3. Tujuan Penelitian, 1.4. Manfaat Penelitian, 1. Aspek Teoritis (Pengembangan Ilmu Pengetahuan), 2. Aspek Praktis (Bagi Masyarakat dan Praktisi), BAB 1, PENDAHULUAN

### Community 44 - "5.3. Saran"
Cohesion: 0.22
Nodes (8): 5.1. Kesimpulan, 5.2. Keterbatasan Penelitian, 5.3.1. Saran Praktis bagi Manajemen Perbankan (Khususnya Bank KBMI 4), 5.3.2. Saran Kebijakan bagi Regulator (Otoritas Jasa Keuangan & Bank Indonesia), 5.3.3. Saran Akademis bagi Peneliti Selanjutnya, 5.3. Saran, BAB 5, PENUTUP

### Community 45 - "Analysis Specification — Spesifikasi Ekonometrika Data Panel"
Cohesion: 0.22
Nodes (8): 1. Spesifikasi Model Matematis & Ekonometrika, 2. Pohon Keputusan Pemilihan Model (Model Selection Protocol), 3. Matriks Diagnostik Uji Asumsi Klasik, 4. Templat Tabel Pelaporan Hasil Analisis (Bab 4), Analysis Specification — Spesifikasi Ekonometrika Data Panel, Tabel 4.X: Hasil Estimasi Regresi Data Panel (Fixed Effect Model), Tabel 4.Y: Efek Spesifik Masing-Masing Bank (*Cross-Section Fixed Effects*), Tabel 4.Z: Ringkasan Uji Kelayakan Model

### Community 46 - "Reviewer Skripsi UKRIDA"
Cohesion: 0.22
Nodes (8): Alur kerja, Aturan bukti, Cakupan minimum, Peran, Persiapan audit, Reviewer Skripsi UKRIDA, Skala prioritas, Urutan otoritas

### Community 47 - "Daftar pemeriksaan"
Cohesion: 0.22
Nodes (8): Audit Format dan Kepatuhan Dokumen, Cara mencatat temuan, Daftar dan lampiran, Daftar pemeriksaan, Prinsip, Struktur dokumen, Tabel, gambar, dan persamaan, Tata letak halaman

### Community 48 - "Audit Isi dan Metodologi"
Cohesion: 0.22
Nodes (9): Analisis statistik, Audit Isi dan Metodologi, Bab pendahuluan, Desain penelitian, Hasil dan pembahasan, Jika menggunakan SEM-PLS, Kesimpulan dan saran, Konsistensi rancangan penelitian (+1 more)

### Community 49 - "Search Log Skripsi — Protokol Pencarian Literatur"
Cohesion: 0.22
Nodes (8): 1. Database & Sumber Pencarian, 2. Kueri Pencarian (Search Strings), 3. Kriteria Inklusi & Eksklusi, 4. Hasil Screening & Seleksi Literatur, 5. Ringkasan Temuan Empiris, Kriteria Eksklusi (Ditolak), Kriteria Inklusi (Diterima), Search Log Skripsi — Protokol Pencarian Literatur

### Community 50 - "Master Revision Plan: Graph of Agents (GoA) Orchestration"
Cohesion: 0.33
Nodes (4): 1. Multi-Agent Topology & DAG Workflow, 3. Dual-Ledger Coordination System, 4. Barrier Policy & Conflict Resolution Matrix, Master Revision Plan: Graph of Agents (GoA) Orchestration

### Community 51 - "2.1. Teori-teori"
Cohesion: 0.25
Nodes (8): 2.1.1. Teori Pemangku Kepentingan (*Stakeholder Theory*), 2.1.2. Teori Legitimasi (*Legitimacy Theory*), 2.1.3. Teori Intermediasi Finansial (*Financial Intermediation Theory*), 2.1.4. Konsep Profitabilitas Bank dan *Return on Assets* (ROA), 2.1.5. Portofolio Kredit Hijau (*Green Financing*), 2.1.6. Risiko Kredit dan *Non-Performing Loan* (NPL), 2.1.7. Kecukupan Modal dan *Capital Adequacy Ratio* (CAR), 2.1. Teori-teori

### Community 52 - "Matriks Literatur Skripsi"
Cohesion: 0.25
Nodes (7): Audit Calon Kontrol, Landasan Teoretis Kerja, Matriks Bukti, Matriks Literatur Skripsi, Protokol Ringkas, Sintesis dan Gap yang Diizinkan, Status

### Community 53 - "Register Input dan Bukti Manusia — Fase 3"
Cohesion: 0.25
Nodes (7): 1. Fungsi Dokumen, 2. Urutan Penguncian, 3. Paket Input Pertama: Akses Lapangan, 4. Paket Input Kedua: Periode dan Perangkat, 5. Paket Input Ketiga: Etik dan Reviewer, 6. Aturan Pembaruan Status, Register Input dan Bukti Manusia — Fase 3

### Community 54 - "Econometric Results Master Summary (v2.0)"
Cohesion: 0.29
Nodes (6): 1. Descriptive Statistics, 2. Correlation Matrix, 3. Multicollinearity (VIF), 4. LSDV Model Estimation (FEM), 5. Elasticities, Econometric Results Master Summary (v2.0)

### Community 55 - "06_RINGKASAN_PER_BAB_ONE_PAGER.md"
Cohesion: 0.29
Nodes (6): 📋 ONE-PAGER BAB 1: PENDAHULUAN, 📋 ONE-PAGER BAB 2: TINJAUAN PUSTAKA & PENGEMBANGAN HIPOTESIS, 📋 ONE-PAGER BAB 3: METODE PENELITIAN, 📋 ONE-PAGER BAB 4: ANALISIS DAN PEMBAHASAN, 📋 ONE-PAGER BAB 5: PENUTUP, 📄 RINGKASAN PER BAB — ONE-PAGER (Bisa Dicetak & Dibawa Terpisah)

### Community 56 - "8. METODOLOGI PENELITIAN & EKONOMETRIKA"
Cohesion: 0.29
Nodes (7): 8.1. Populasi & Sampel (*Purposive Sampling*), 8.2. Mengapa Menggunakan Regresi Data Panel?, 8.3. Model Regresi, 8.4. Tahap Penentuan Model Terbaik (Model Selection Tests), 8.5. Hasil Uji Asumsi Klasik (Semua Wajib Lolos!), 8.6. Alat & Metode Analisis, 8. METODOLOGI PENELITIAN & EKONOMETRIKA

### Community 57 - "PENDAHULUAN"
Cohesion: 0.29
Nodes (7): 1.1. Latar Belakang Penelitian, 1.2. Perumusan Masalah, 1.3. Tujuan Penelitian, 1.4. Manfaat Penelitian, 1. Aspek Teoritis, 2. Aspek Praktis, PENDAHULUAN

### Community 58 - "METODE PENELITIAN"
Cohesion: 0.29
Nodes (7): 3.1. Jenis dan Sumber Data, 3.2. Populasi dan Sampel, 3.3. Model Penelitian, 3.4. Operasionalisasi Variabel, 3.5. Metode Analisis Data, METODE PENELITIAN, Tabel 3.2: Operasionalisasi Variabel

### Community 59 - "Tugas: Buat Website Portofolio Sederhana (HTML + CSS + JS vanilla)"
Cohesion: 0.29
Nodes (6): Desain, Halaman & section (index.html, satu halaman dengan smooth-scroll nav), Konten Sumber (FAKTA dari skripsi — jangan mengarang angka lain), Struktur file (buat persis seperti ini), Tugas: Buat Website Portofolio Sederhana (HTML + CSS + JS vanilla), Verifikasi wajib sebelum selesai

### Community 60 - "Tugas: Percantik Website Portofolio Skripsi (iterasi 2)"
Cohesion: 0.29
Nodes (6): 1. Identitas pemilik (WAJIB), 2. Halaman publikasi terpisah (BARU), 3. Ganti skema warna (refresh visual), 4. Polish tambahan, Tugas: Percantik Website Portofolio Skripsi (iterasi 2), Verifikasi wajib

### Community 61 - "SKRIPSI_ARTHUR_LENGKAP_PRISM.md"
Cohesion: 0.33
Nodes (4): SKRIPSI LENGKAP - UNIVERSITAS KRISTEN KRIDA WACANA, 📂 Panduan Struktur Folder, 🚀 Quick Navigation, Workspace Skripsi FEB UKRIDA

### Community 62 - "1. RINGKASAN EKSEKUTIF & PETA KONSEP PENELITIAN"
Cohesion: 0.67
Nodes (3): 1. RINGKASAN EKSEKUTIF & PETA KONSEP PENELITIAN, Mengapa Penelitian Ini Dibuat?, Peta Hubungan Variabel:

### Community 63 - "Data Dictionary & Audit Dataset Skripsi"
Cohesion: 0.33
Nodes (5): 1. Identifikasi Objek & Kerangka Sampel (Purposive Sampling), 2. Definisi Operasional Variabel & Formula Matematis, 3. Struktur Dataset Panel (Panel Data Design), 4. Protokol Ekstraksi & Integritas Data, Data Dictionary & Audit Dataset Skripsi

### Community 64 - "Source of Truth Skripsi — Status Lengkap (Bab 1–5)"
Cohesion: 0.33
Nodes (5): 1. Identitas & Standar Ketebalan, 2. Struktur Variabel & Operasionalisasi, 3. Ringkasan Pembuktian Hipotesis, 4. Alokasi Standar Ketebalan Bab ($\ge 80$ Halaman), Source of Truth Skripsi — Status Lengkap (Bab 1–5)

### Community 65 - "06_Review_&_Audit/README.md"
Cohesion: 0.33
Nodes (3): 📋 Daftar Berkas Review & Audit, 🎯 Panduan Penggunaan, Review & Audit Skripsi

### Community 66 - "Audit Bahasa, Sitasi, dan Daftar Pustaka"
Cohesion: 0.33
Nodes (6): Audit Bahasa, Sitasi, dan Daftar Pustaka, Bahasa akademik, Bentuk rekomendasi redaksi, Istilah dan konsistensi, Rekonsiliasi daftar pustaka, Sitasi dalam teks

### Community 67 - "📝 BANK SOAL LATIHAN SKRIPSI — 30 SOAL (PG + ESSAY)"
Cohesion: 0.40
Nodes (4): BAGIAN A: PILIHAN GANDA (20 Soal), BAGIAN B: SOAL ESSAY (10 Soal), 📝 BANK SOAL LATIHAN SKRIPSI — 30 SOAL (PG + ESSAY), *Seluruh angka diambil persis dari naskah skripsi BAB 1–5*

### Community 68 - "7. LOGIKA HUBUNGAN KAUSALITAS & HIPOTESIS"
Cohesion: 0.40
Nodes (5): 1. Pengaruh Green Financing terhadap ROA ($\mathbf{H_1}$: Positif & Signifikan), 2. Pengaruh NPL terhadap ROA ($\mathbf{H_2}$: Negatif & Signifikan), 3. Pengaruh CAR terhadap ROA ($\mathbf{H_3}$: Positif & Signifikan), 4. Pengaruh Simultan GF, NPL, dan CAR terhadap ROA ($\mathbf{H_4}$: Simultan Signifikan), 7. LOGIKA HUBUNGAN KAUSALITAS & HIPOTESIS

### Community 69 - "3. KAMUS LENGKAP ISTILAH & KONSEP DASAR"
Cohesion: 0.40
Nodes (5): 3. KAMUS LENGKAP ISTILAH & KONSEP DASAR, A. Lembaga & Klasifikasi, B. Istilah Operasional & Risiko Bank, C. Istilah Keberlanjutan (*Sustainability & ESG*), D. Istilah Statistik/Ekonometrika Dasar (agar tidak "blank" saat ditanya angka)

### Community 70 - "9. HASIL ANALISIS DATA & INTERPRETASI ANGKA"
Cohesion: 0.40
Nodes (5): 9.1. Statistik Deskriptif ($N = 80$), 9.2. Persamaan Regresi Panel (Fixed Effect Model), 9.3. Tabel Rangkuman Uji Parsial (Uji t) & Simultan (Uji F), 9.4. Cara Membaca Angka-Angka Ini (Gaya Mahasiswa Manajemen), 9. HASIL ANALISIS DATA & INTERPRETASI ANGKA

### Community 71 - "TINJAUAN PUSTAKA DAN PENGEMBANGAN HIPOTESIS"
Cohesion: 0.40
Nodes (5): 2.2. Penelitian Sebelumnya, 2.3. Rerangka Penelitian, 2.4. Pengembangan Hipotesis, Tabel 2.1: Matriks Penelitian Sebelumnya, TINJAUAN PUSTAKA DAN PENGEMBANGAN HIPOTESIS

### Community 72 - "PRISM AI Technical Review & Comprehensive Audit Synthesis"
Cohesion: 0.40
Nodes (4): 1. Executive Summary, 2. Taksonomi Temuan Definitif (12 Critical Findings & Editorial Issues), 3. Temuan Editorial & *Proofing Defects* Tambahan, PRISM AI Technical Review & Comprehensive Audit Synthesis

### Community 73 - "Workflow Log Skripsi — Topik Aktif"
Cohesion: 0.40
Nodes (4): 10 Agustus 2026 — Pembatalan Topik Lama & Reset, 19 Agustus 2026 — Penetapan Topik Baru & Pembukaan Fase 4, 19 Agustus 2026 — Penyelesaian Utuh Naskah Skripsi Lengkap (Bab 1 s.d. Bab 5), Workflow Log Skripsi — Topik Aktif

### Community 74 - "11. KESIMPULAN, KETERBATASAN, & SARAN"
Cohesion: 0.50
Nodes (4): 11.1. Kesimpulan (Ringkas 4 Poin — Sejajar dengan H1–H4), 11.2. Keterbatasan Penelitian (WAJIB dihafal — hampir selalu ditanya!), 11.3. Saran, 11. KESIMPULAN, KETERBATASAN, & SARAN

### Community 75 - "2. RUMUSAN MASALAH, TUJUAN & MANFAAT PENELITIAN"
Cohesion: 0.50
Nodes (4): 2.1. Perumusan Masalah (4 Pertanyaan Riset), 2.2. Tujuan Penelitian, 2.3. Manfaat Penelitian, 2. RUMUSAN MASALAH, TUJUAN & MANFAAT PENELITIAN

### Community 76 - "PENUTUP"
Cohesion: 0.50
Nodes (4): 5.1. Kesimpulan, 5.2. Keterbatasan Penelitian, 5.3. Saran, PENUTUP

### Community 77 - "Literature Matrix — Sintesis Literatur Skripsi"
Cohesion: 0.50
Nodes (3): 1. Matriks Artikel Jurnal Empiris (Peer-Reviewed), 2. Rujukan Buku Teks Utama (Grand & Middle Theory), Literature Matrix — Sintesis Literatur Skripsi

### Community 78 - "Format Laporan Hasil Review"
Cohesion: 0.50
Nodes (3): Format Laporan Hasil Review, Gaya penulisan laporan, Prinsip keluaran

### Community 79 - "update_identity_all_files.py"
Cohesion: 0.67
Nodes (3): main(), Script to update Author Name (Arthur Reezan) and NIM (312023002) across all…, update_file()

### Community 80 - "🎓 PANDUAN BELAJAR & PENGUASAAN MATERI SKRIPSI (MASTER STUDY GUIDE) — v2"
Cohesion: 0.67
Nodes (3): 📌 DAFTAR ISI PANDUAN, 🎓 PANDUAN BELAJAR & PENGUASAAN MATERI SKRIPSI (MASTER STUDY GUIDE) — v2, *Program Studi S1 Manajemen — Konsentrasi Manajemen Keuangan — UKRIDA*

### Community 83 - "KATA PENGANTAR"
Cohesion: 0.67
Nodes (3): KATA PENGANTAR, Universitas Kristen Krida Wacana | vii, Universitas Kristen Krida Wacana | viii

### Community 93 - "generate_template_word.py"
Cohesion: 0.22
Nodes (37): add_body_text(), add_figure_caption(), add_footer_latin(), add_footer_romawi(), add_heading_bab(), add_page_break(), add_paragraph(), add_placeholder_note() (+29 more)

### Community 94 - "sync_markdown_from_tex.py"
Cohesion: 0.67
Nodes (3): clean_latex_to_md(), main(), Script to synchronize and update SKRIPSI_UKRIDA_LENGKAP_PRISM.md from the…

### Community 95 - "📥 FOLDER DROPZONE PENGUMPULAN LAPORAN KEUANGAN & SUSTAINABILITY REPORT"
Cohesion: 0.20
Nodes (8): 🌟 1. Jalur Tercepat & Terlengkap (One-Stop Portal BEI / IDX):, 🏛️ 2. Tautan Langsung Portal Investor Relations Masing-Masing Bank:, 💡 Cara Pakai:, 📥 FOLDER DROPZONE PENGUMPULAN LAPORAN KEUANGAN & SUSTAINABILITY REPORT, 🌐 REKOMENDASI TAUTAN UNDUHAN RESMI:, 1. Matriks Laporan Keuangan Kuartalan (80 Observasi), 2. Matriks Sustainability Report Tahunan (20 Laporan), 📊 Status Inventaris Laporan Keuangan & Keberlanjutan KBMI 4 (2021–2025)

### Community 96 - "organize_and_audit_reports.py"
Cohesion: 0.43
Nodes (7): extract_pdf_snippet(), generate_inventory(), identify_bank(), identify_type_and_period(), identify_year(), organize_all(), Path

## Knowledge Gaps
- **1371 isolated node(s):** `SKRIPSI LENGKAP - UNIVERSITAS KRISTEN KRIDA WACANA`, `Dinamika Makroekonomi dan Transisi Pasca-Pandemi Periode 2021–2025`, `Perbandingan Kerangka Regulasi Keuangan Berkelanjutan Global dan Regional`, `Peran Sistemik Bank KBMI 4 dalam Perekonomian Nasional`, `Perdebatan Empiris dan Teoretis (*Research Gap` (+1366 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **11 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `BAB V  PENUTUP ........................................................................................... 56` connect `BAB V  PENUTUP ........................................................................................... 56` to `:`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `:` connect `:` to `BAB V  PENUTUP ........................................................................................... 56`, `KATA PENGANTAR`, `ABSTRAK`, `BAB II TINJAUAN PUSTAKA ............................................................................. 9`, `BAB IV ANALISIS DATA DAN PEMBAHASAN ............................................. 37`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `(322015175)` connect `(322015175)` to `Buku Pedoman Penyusunan Skripsi FEB Ukrida 2022.md`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **What connects `SKRIPSI LENGKAP - UNIVERSITAS KRISTEN KRIDA WACANA`, `Dinamika Makroekonomi dan Transisi Pasca-Pandemi Periode 2021–2025`, `Perbandingan Kerangka Regulasi Keuangan Berkelanjutan Global dan Regional` to the rest of the system?**
  _1371 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `BAB V  PENUTUP ........................................................................................... 56` be split into smaller, more focused modules?**
  _Cohesion score 0.004842615012106538 - nodes in this community are weakly interconnected._
- **Should `BAB 4\\[0.3cm]ANALISIS DAN PEMBAHASAN` be split into smaller, more focused modules?**
  _Cohesion score 0.05405405405405406 - nodes in this community are weakly interconnected._
- **Should `Konseptualisasi Variabel Penelitian` be split into smaller, more focused modules?**
  _Cohesion score 0.08 - nodes in this community are weakly interconnected._